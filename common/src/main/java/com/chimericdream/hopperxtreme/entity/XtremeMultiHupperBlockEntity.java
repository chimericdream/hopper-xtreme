...
public class XtremeMultiHupperBlockEntity extends LootableContainerBlockEntity implements Hopper {
    ...
    public XtremeMultiHupperBlockEntity(BlockPos pos, BlockState state, int cooldownInTicks) {
        super(XTREME_MULTI_HUPPER_BLOCK_ENTITY.get(), pos, state);
        ...
        this.northConnected = state.get(XtremeMultiHupperBlock.NORTH_CONNECTED);
        this.southConnected = state.get(XtremeMultiHupperBlock.SOUTH_CONNECTED);
    ...
    protected Text getContainerName() {
        ...
        if (block instanceof XtremeMultiHupperBlock) {
            String baseKey = ((XtremeMultiHupperBlock) block).getBaseKey();
    ...
    public static void serverTick(World world, BlockPos pos, BlockState state, XtremeMultiHupperBlockEntity blockEntity) {
    ...
    private static int getCooldownForBlock(XtremeMultiHupperBlockEntity blockEntity) {
        Block block = blockEntity.getCachedState().getBlock();

        if (block instanceof XtremeMultiHupperBlock) {
            return ((XtremeMultiHupperBlock) block).getCooldownInTicks();
        }
    ...
    private static boolean insertAndExtract(World world, BlockPos pos, BlockState state, XtremeMultiHupperBlockEntity blockEntity, BooleanSupplier booleanSupplier) {
        if (world.isClient) {
            return false;
        }

        if (!blockEntity.needsCooldown() && state.get(XtremeMultiHupperBlock.ENABLED)) {
            boolean bl = false;
    ...
    private static boolean insert(World world, BlockPos pos, XtremeMultiHupperBlockEntity blockEntity) {
    ...
    public static boolean extract(World world, Hopper hupper) {
        ...
        boolean bl = ((XtremeMultiHupperBlockEntity) hupper).canBlockFromBelow() && blockState.isFullCube(world, blockPos) && !blockState.isIn(BlockTags.DOES_NOT_BLOCK_HOPPERS);
        if (!bl) {
    ...
    private static ItemStack transfer(@Nullable Inventory from, Inventory to, ItemStack stack, int slot, @Nullable Direction side) {
        ...
        if (canInsert(to, stack, slot, side)) {
            ...
            if (bl) {
                if (bl2 && to instanceof XtremeMultiHupperBlockEntity hopperBlockEntity) {
                    if (!hopperBlockEntity.isDisabled()) {
                        int j = 0;

                        if (from instanceof XtremeMultiHupperBlockEntity hopperBlockEntity2) {
    ...
    public static void onEntityCollided(World world, BlockPos pos, BlockState state, Entity entity, XtremeMultiHupperBlockEntity blockEntity) {
